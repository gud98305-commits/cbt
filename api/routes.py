...
from trade_license_cbt.services.pdf_parser import (
    parse_pdf, parse_answer_pdf, merge_answers
)
...
async def set_api_key(body: ApiKeyBody, request: Request):
    ...
    session.put(_sid(request), "api_key", key)
    return {"ok": True}


@router.post("/api/parse-pdf")
async def api_parse_pdf(request: Request, file: UploadFile = File(...)):
    sid = _sid(request)
    api_key = session.get(sid, "api_key")
    if not api_key:
    ...
    try:
        questions = await asyncio.to_thread(parse_pdf, file_bytes, api_key)
    ...
    session.put(sid, "parsed_questions", questions)
    return {"count": len(questions), "ok": True}


@router.post("/api/parse-answer")
async def api_parse_answer(request: Request, file: UploadFile = File(...)):
    sid = _sid(request)
    api_key = session.get(sid, "api_key")
    if not api_key:
        raise HTTPException(status_code=400, detail="API 키가 설정되지 않았습니다.")
    ...
    if len(file_bytes) > MAX_PDF_SIZE:
        raise HTTPException(status_code=413, detail="PDF 파일이 너무 큽니다 (최대 50MB).")
    try:
        answers = await asyncio.to_thread(parse_answer_pdf, file_bytes, api_key)
    except (ValueError, RuntimeError) as e:
        raise HTTPException(status_code=422, detail=str(e))
    if not answers:
        raise HTTPException(status_code=422, detail="답안을 추출하지 못했습니다.")

    session.put(sid, "parsed_answers", answers)
...
async def session_status(request: Request):
    sid = _sid(request)
    pq: list[Question] = session.get(sid, "parsed_questions", [])
    pa: list[dict] = session.get(sid, "parsed_answers", [])
    subjects = sorted(set(q.subject for q in pq if q.subject)) if pq else []
    return {
        "question_count": len(pq),
        "answer_count": len(pa),
        "subjects": subjects,
        "api_key_set": bool(session.get(sid, "api_key")),
    }


@router.post("/api/start-exam")
async def start_exam(body: StartExamBody, request: Request):
    sid = _sid(request)
    parsed_q: list[Question] = session.get(sid, "parsed_questions", [])
    parsed_a: list[dict] = session.get(sid, "parsed_answers", [])
    ...
    session.put(sid, "questions", final_questions)
    session.put(sid, "exam_state", ExamState())
    return {"total": len(final_questions), "ok": True}


@router.post("/api/retry-exam")
async def retry_exam(request: Request):
    sid = _sid(request)
    questions: list[Question] = session.get(sid, "questions", [])
    if not questions:
...
async def start_sample_exam(request: Request):
    sid = _sid(request)
    session.put(sid, "questions", SAMPLE_QUESTIONS)
    session.put(sid, "exam_state", ExamState())
...
async def get_exam_state(request: Request):
    sid = _sid(request)
    exam_state: ExamState = session.get(sid, "exam_state")
    questions = session.get(sid, "questions", [])
    if not exam_state:
...
async def save_answer(body: SaveAnswerBody, request: Request):
    sid = _sid(request)
    exam_state: ExamState = session.get(sid, "exam_state")
    if not exam_state:
        raise HTTPException(status_code=404, detail="시험 세션이 없습니다.")
...
async def navigate(body: NavigateBody, request: Request):
    sid = _sid(request)
    exam_state: ExamState = session.get(sid, "exam_state")
    questions = session.get(sid, "questions", [])
    if not exam_state:
...
async def submit_exam(request: Request):
    ...
    session.put(sid, "final_score", score)
    session.put(sid, "incorrect_questions", incorrect)
...
async def get_results(request: Request):
    sid = _sid(request)
    score = session.get(sid, "final_score", 0.0)
    questions = session.get(sid, "questions", [])
    incorrect = session.get(sid, "incorrect_questions", [])
    exam_state: ExamState = session.get(sid, "exam_state")
